...
又是可爱的树上 dp，不过这次是取最值，有点搞（求和的话比较简单，最大最小值，一般需要次大小值）

> 这题要注意一下自己不能到自己

设 `dist[u]` 表示以 $u$ 为根的子树中，$u$ 到其它节点距离的最大值（带点权重 d），设 `ans[u]` 为 $u$ 对应的答案，那么如果 u 是 v 的父节点，且 ans[u] 是 u 到 v 的子树的，那么就麻烦了，就无法更新出答案，因此我们有必要保存出次优答案。注意这里次优和主优要在 u 的不同子树上

更详细的说，`dist[u]` 表示以 $u$ 为根的子树对应的答案，`ddist[u]` 表示以 $u$ 为根的子树对应的次优答案，同样，我们令 `ans[u]` 表示最终答案，`aans[u]` 表示此有答案。**要求答案和次优答案的最短路必然经过 $u$**

那么显然 $ans[root] = dist[root]$, $aans[root] = ddist[root]$，然后如果 $u$ 是 $v$ 的父节点，且已经知道 $ans[u], aans[u]$，如果 $ans[u] = \max(dist[v], d[v]) + w$（这就说明 u 的最优点落在了 v 的子树中），所以此时我们应该使用 `aans[u]`，否则我们就应该使用 `ans[u]`
