...
int main() {
  ...
  std::cin >> n >> k;
  // 为啥这么慢啊
  std::vector<Mod> fac(k + 1), ifac(k + 1);
  fac[0] = fac[1] = 1;
  for (int i = 2; i <= k; ++i) fac[i] = fac[i - 1] * Mod::raw(i);
  ifac[k] = fac[k].inv();
  for (int i = k; i > 0; --i) ifac[i - 1] = ifac[i] * Mod::raw(i);
  auto f = ifac, g = ifac;
  for (int i = 1; i <= k; i += 2) f[i] = -f[i];
  for (int i = 0; i <= k; ++i) g[i] *= pow(Mod(i), k);
  auto S = (PolyNTT(f) * PolyNTT(g)).modXn(k + 1);
